...
@router.post('/usuario', tags=["user"], response_model=UserSimple, status_code=status.HTTP_201_CREATED)
...
@router.get('/users', tags=["user"], response_model=List[SchemasUser], status_code=status.HTTP_200_OK)
...
def get_user(id: int, db:Session = Depends(get_db)):
    ...
    if not user_located:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail= f"There isn't user with id = {id}")
    return user_located
...
def remove_user(id: int, db: Session = Depends(get_db)):
    ...
    return JSONResponse({"detail": "User deleted successfully",
                        "data": id}, status_code=status.HTTP_200_OK)